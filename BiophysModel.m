function [params]=BiophysModel(params)
    ...
    %       - h: Cooperativity for the relation of cGMP to current.
    %       - cdark: Calcium concentration in darkness.
    %       - beta: Rate constant for calcium removal (1/sec).
    %       - hillcoef: Cooperativity for the cyclase (Hill coefficient).
    %       - hillaffinity: Hill affinity for the cyclase.
...
if (params.biophysFlag == 1)
    ...
    params.gdark = (2 * params.darkCurrent / params.k)^(1/params.h);
    ...
    for pnt = 2:NumPts
        ...
        c(pnt) = c(pnt-1) + TimeStep * (cur2ca * params.k * g(pnt-1)^params.h - params.beta * c(pnt-1));
    ...
    params.response = -params.k * g.^params.h;
else   % linear
